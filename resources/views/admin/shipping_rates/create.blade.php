...
@section('header_title', 'Add Shipping Rate')

@section('content')
<div class="brand-card">
    <div class="card-header">
        <h2><i class="fa-solid fa-plus"></i> Add New Shipping Rate</h2>
    </div>

    <form action="{{ route('admin.shipping-rates.store') }}" method="POST" style="padding: 20px;">
        @csrf
        
        <div class="form-group">
            <label>City / Region Name</label>
            <input type="text" name="region_name" class="form-control" value="{{ old('region_name') }}" placeholder="e.g. Bago" required>
            @error('region_name') <span class="error-msg">{{ $message }}</span> @enderror
        </div>

        <div class="form-group">
            <label>Base Fee (Ks) [Covers first item]</label>
            <input type="number" name="base_fee" class="form-control" value="{{ old('base_fee', 0) }}" min="0" required>
            @error('base_fee') <span class="error-msg">{{ $message }}</span> @enderror
        </div>

        <div class="form-group">
            <label>Extra Fee Per Item (Ks) [For 2nd item onwards]</label>
            <input type="number" name="extra_fee_per_item" class="form-control" value="{{ old('extra_fee_per_item', 0) }}" min="0" required>
            @error('extra_fee_per_item') <span class="error-msg">{{ $message }}</span> @enderror
        </div>

        <button type="submit" class="btn-submit">Save Rate</button>
