...
@section('header_title', 'Update Staff Account')
...
<div class="form-card">
    <div class="card-header">
        <h2><i class="fa-solid fa-user-pen"></i> Update: {{ $staff->staff->name ?? $staff->email }}</h2>
    </div>

    <div class="card-body">
        @if($errors->any())
            <div class="alert-error">
                <ul class="inline-style-41">
        ...
        <form action="{{ route('admin.staff.update', $staff->id) }}" method="POST">
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <input type="text" id="name" name="name" class="form-control" value="{{ old('name', $staff->staff->name ?? '') }}" required>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="phone" name="phone" class="form-control" value="{{ old('phone', $staff->staff->phone ?? '') }}">
                </div>

                <div class="form-group">
                    <label for="password">New Password (Leave blank to keep current)</label>
                    <div class="password-wrapper">
                        <input type="password" id="password" name="password" class="form-control" minlength="8" placeholder="••••••••">
                ...
                <div class="form-group">
                    ...
                    <select id="status" name="status" class="form-control" required>
                        <option value="active" {{ old('status', $staff->status) == 'active' ? 'selected' : '' }}>Active (Can Login)</option>
                        <option value="inactive" {{ old('status', $staff->status) == 'inactive' ? 'selected' : '' }}>Inactive (Suspended)</option>
                    </select>
                </div>
            </div>
