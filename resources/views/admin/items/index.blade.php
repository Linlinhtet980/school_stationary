        ...
        <table class="item-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="price-column">{{ number_format($item->price, 2) }} Ks</td>
                    <td>
                        @if($item->stock_quantity <= 0)
                            <span class="stock-out">Out of Stock</span>
                        @elseif($item->stock_quantity <= 10)
                            <span class="stock-low">{{ $item->stock_quantity }} (Low)</span>
                        @else
                            <span class="stock-normal">{{ $item->stock_quantity }}</span>
                        @endif
                    </td>
                    <td>
                    ...
                    <td>
                        <div class="action-btns">
                            <a href="{{ route('admin.items.show', $item->id) }}" class="btn-icon btn-view" title="View Details">
                                <i class="fa-solid fa-eye"></i>
                            </a>
                            <a href="{{ route('admin.items.edit', $item->id) }}" class="btn-icon btn-edit" title="Edit Item">
                                <i class="fa-solid fa-pen"></i>
                            </a>
                            <form action="{{ route('admin.items.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this item?');" class="form-inline">
                                @csrf
                                @method('DELETE')
