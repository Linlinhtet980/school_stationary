    ...
    <main class="profile-main">
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><a href="{{ route('profile.order-detail', $order->id) }}" class="btn-view">View Details</a></td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
        ...
        <div id="wishlist-section" class="section-box">
            ...
            <div class="wishlist-grid">
                ...
                <div class="wishlist-card">
                    <form action="{{ route('profile.remove-wishlist', $wish->id) }}" method="POST" style="position:absolute; top:10px; right:10px;">
                        @csrf @method('DELETE')
                        <button type="submit" class="wishlist-remove" title="Remove"><i class="fa-solid fa-times"></i></button>
                    </form>
                    <img src="{{ $wish->item->images->first() ? asset('storage/' . $wish->item->images->first()->image_path) : asset('images/placeholder.jpg') }}" alt="{{ $wish->item->name }}">
        ...
        <div id="addresses-section" class="section-box">
            <div class="inline-style-108">
                ...
                <button class="btn-add-address" onclick="toggleModal('addressModal')"><i class="fa-solid fa-plus"></i> Add New</button>
            </div>
            ...
            <div class="address-grid-2col">
                ...
                <div class="address-card {{ $address->is_default ? 'default' : '' }}">
                    ...
                    <div class="address-actions">
                        <button onclick="editAddress({{ $address->id }}, '{{ addslashes($address->label) }}', '{{ addslashes($address->address_line) }}', '{{ addslashes($address->city) }}', '{{ addslashes($address->phone) }}')">Edit</button>
                        @if(!$address->is_default)
                        <form action="{{ route('profile.set-default-address', $address->id) }}" method="POST">
                            @csrf
                            <button type="submit">Set Default</button>
                        </form>
                        <form action="{{ route('profile.delete-address', $address->id) }}" method="POST" onsubmit="return confirm('Delete this address?')">
                            @csrf @method('DELETE')
        ...
        <div id="account-section" class="section-box">
            ...
            <div class="profile-image-row">
                ...
                <form action="{{ route('profile.update-image') }}" method="POST" enctype="multipart/form-data" class="inline-style-110">
                    @csrf
                    <input type="file" name="image" id="profileImageInput" accept="image/*" class="inline-style-111" onchange="this.form.submit()">
                    <button type="button" class="btn-change-photo" onclick="document.getElementById('profileImageInput').click()">
    ...
    <div class="modal-content">
        ...
        <form action="{{ route('profile.add-address') }}" method="POST" id="addressForm">
            ...
            <div class="form-group-p">
                ...
                <textarea name="address_line" id="addressLine" rows="2" required placeholder="No.12, Zay Street..." class="inline-style-117"></textarea>
            </div>
            <div class="form-row-2">
                <div class="form-group-p">
                ...
                <div class="form-group-p">
                    ...
                    <input type="text" name="city" id="addressCity" required list="profileRegionOptions" placeholder="Select or type region...">
